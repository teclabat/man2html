...
# NAME_file  - Array indexed by NAME and containing file names used for hyperlinks.
# KEY_file   - Array indexed by KEYWORD and containing file names used for hyperlinks.
...
# dash --
#
# This procedure is invoked to handle dash characters ("\-" in troff).
# Its only function in pass1 is to terminate the NAME state.
#
# Arguments:
# None.
...
# Its only purpose is to terminate a DT (dictionary term).
#
# Arguments:
# None.
...
# doContents --
#
# Generates an HTML contents file using the NAME_file array
...
# do --
#
# This is the toplevel procedure that searches a man page for hypertext links.
# It builds a database consisting of two arrays: NAME_file and KEY_file.
# It runs the man2tcl program to turn the man page into a script, then evals
# that script.
#
# Arguments:
...
proc do {fileName} {
    global curFile scriptDir
    set curFile [file tail $fileName]
    set file stdout
    puts "  Pass 1 -- $fileName"
    flush stdout

    # Construct path to man2tcl executable
    set man2tcl [file join $scriptDir man2tcl]
